...
;Used to return number of positions where lists differ
(define (hamming-distance lst1 lst2)               
  (cond ((null? lst1) (null? lst2) 0)                     ;checks if the lists are empty
        ((not(= (car lst1) (car lst2)))                   ;checks if element in list are the same
         (+ 1 (hamming-distance (cdr lst1) (cdr lst2))))  ;if same, add 1 and recur
