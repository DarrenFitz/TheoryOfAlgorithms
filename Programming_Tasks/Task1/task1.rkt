...
(define (decide-prime x)
  ;indivisible funct used to check x is only divisable by 1 and itself
  (define (indivisible-by a b)
    ;if b = 1, proving x only divisable by 1 and itself 
    (cond
     ((= b 1) #T)
     (else (if(= (modulo a b) 0)
          #F
          (indivisible-by a (- b 1))))))
  ;If prime is equal to 1 return false, 1 is not a prime number
  ;else -1 from x 
  (if (= x 1)
      #F
